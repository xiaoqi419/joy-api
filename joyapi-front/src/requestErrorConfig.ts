  ...
  errorConfig: {
    ...
    errorHandler: (error: any, opts: any) => {
      ...
      if (error.name === 'BizError') {
        ...
        AnDMessage.error(`Response status:${error.response.status}`)
      } else if (error.request) {
        // 请求已经成功发起，但没有收到响应
        // \`error.request\` 在浏览器中是 XMLHttpRequest 的实例，
        // 而在node.js中是 http.ClientRequest 的实例
        AnDMessage.error('None response! Please retry.')
      } else {
        // 发送请求时出了点问题
        AnDMessage.error('Request error, please retry.')
      }
    }
  },
